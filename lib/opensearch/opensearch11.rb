...
module OpenSearch
  class OpenSearch11 < OpenSearchBase
    Nodes = {
    ...
    def setup_description(doc)
      Nodes.each_key do |node|
        node_camel_case = node.gsub(/(^|_)(.)/){$2.upcase}

        doc.children.xpath(".//xmlns:#{node_camel_case}").each do |n|
          description = nil
          ...
          if Nodes[node][:format].class == Hash
            ...
            if node == 'url' && !n.children.empty?
              description['param'] = ""
              n.xpath('//xmlns:Param').each do |param|
